...
VS Code는 화면이 네 영역으로 나뉩니다. **파일 탐색기**(왼쪽) — **에디터**(가운데) — **채팅창**(오른쪽) — **터미널**(아래). 채팅창에 입력하면 Claude가 실제로 파일을 만들고 수정합니다. 결과는 파일 탐색기에 바로 반영됩니다.
...
Clone이 끝나면 VS Code에서 레포 폴더를 엽니다.

1. VS Code 메뉴에서 `파일 → 폴더 열기...` (영문: `File → Open Folder...`)를 선택합니다.
...
4. VS Code에서 레포를 열고 `/tutorial`로 AI 가이드를 활성화했습니다.
