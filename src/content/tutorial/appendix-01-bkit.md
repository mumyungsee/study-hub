---
title: "부록 H. bkit 심화 — 스킬과 개발 파이프라인"
chapter: 108
order: 0
track: 부록
...
이 튜토리얼에서 쓴 `/pdca plan`, `/pdca analyze`가 bkit 스킬입니다.
...
7장에서 쓴 `/pdca plan`, 9장에서 쓴 `/pdca analyze`가 각각 Plan, Check 단계에 해당합니다.
...
<details>
<summary>에이전트 팀이 뭔지 궁금하다면</summary>

bkit에는 **CTO 에이전트**라는 특별한 역할이 있습니다. 여러 전문 에이전트를 하나의 팀으로 묶어 복잡한 프로젝트를 처리하는 오케스트레이터입니다.
...
CTO 에이전트 (전체 지휘)
  ↓ 역할 분배
  ├── 프론트엔드 에이전트 — UI/컴포넌트 설계
  ├── 백엔드 에이전트    — API/데이터베이스 설계
  ├── 보안 에이전트      — 취약점 검토
  ├── QA 에이전트        — 테스트 전략
  └── 리포트 에이전트    — 결과 종합 보고
```

혼자서 전부 잘 하는 AI 한 명보다, **각 분야 전문가 AI들이 나눠서 검토**하는 구조입니다. 결과물의 품질이 다릅니다.

### 이게 "에이전트 오케스트레이션"인 이유
...
```
나 → CTO 에이전트 → 여러 전문 에이전트 → 각자 작업 → CTO가 종합 → 나
```

CTO가 "지금 이 태스크는 프론트엔드 문제야, 저 태스크는 보안 문제야"라고 판단해서 알맞은 전문가 에이전트에게 넘깁니다. 이 구조를 **멀티 에이전트 오케스트레이션**이라고 합니다.

### 언제 필요한가요

이 튜토리얼 수준(Starter)에서는 필요 없습니다. Enterprise 레벨, 즉 여러 명이 협업하고 보안·인증·인프라가 얽힌 큰 프로젝트에서 진가를 발휘합니다.

```
Starter    → 혼자, AI 한 명과 대화
Dynamic    → 혼자 또는 소규모, 여러 스킬 활용
Enterprise → CTO 에이전트가 팀 전체를 지휘
```

bkit 공식 사이트 → [bkit.ai](https://www.bkit.ai/) 에서 더 자세히 볼 수 있습니다.
...
- 주로 쓰는 스킬: `/pdca plan`, `/pdca analyze`
...
- 사용자 인증 (로그인/회원가입)
- 데이터베이스 연동
- 주로 쓰는 스킬: `/pdca plan`, `/pdca design`, `/pdca analyze`
...
| bkit 본체 플러그인 (오픈소스) | [github.com/popup-studio-ai/bkit-claude-code](https://github.com/popup-studio-ai/bkit-claude-code) |
| bkit-starter (초보자용 한글 가이드) | [github.com/popup-studio-ai/bkit-starter](https://github.com/popup-studio-ai/bkit-starter) |
