...
**명령어를 외워서 직접 치는 것보다 클로드한테 말로 시키는 걸 메인으로 가르친다.**
- 버전 확인, 업데이트, 설치 확인 등 → 클로드한테 자연어로 요청하는 프롬프트가 메인
- 직접 명령어(터미널 입력)는 `<details>` 접기 안에 참고로만
- 예외: `/plugin marketplace add`, `/plugin install` 같이 채팅창에 직접 입력해야만 동작하는 것은 그대로 유지
- 새 페이지 작성하거나 기존 페이지 수정할 때 이 원칙 적용
...
## 톤·문체 가이드 (사이트 본문 기준 — 채팅 톤은 전역 CLAUDE.md)

- 본문은 "~합니다"보다 "~해요" 위주, 막막한 지점을 짚어주는 코칭 톤
...
5. 본문 이모지도 최소 — ⚠️ 같은 경고 마커는 정말 필요한 곳에만
...
- 보류한 작업도 `docs/` 계획서에 기록 (세션 끊겨도 이어갈 수 있게)

### 4. 세션 끝낼 때 루틴 ("맥락 업데이트" 요청 시)
...
- Vercel 자동 배포: GitHub `main` push 시 (운영 URL은 문서 상단)
